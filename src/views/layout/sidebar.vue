<template>
  <aside class="site-sidebar" :class="sidebarClasses">
    <div>
      <el-menu :default-active="menuNavActiveName" :collapse="$store.state.sidebarCollapse" :collapseTransition="false" class="site-sidebar__menu">
        <el-menu-item index="1-1" @click="$router.push({ name: 'home' })">
          <icon-svg name="shouye" class="site-sidebar__menu-icon"></icon-svg>
          <span slot="title">首页</span>
        </el-menu-item>
        <sub-menu-nav v-for="menuNav in $store.state.menuNavList" :key="menuNav.menuId" :menu-nav="menuNav">
        </sub-menu-nav>
      </el-menu>
    </div>
  </aside>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      ...mapMutations(['UPDATE_MENU_NAV_LIST', 'UPDATE_MENU_NAV_ACTIVE_NAME', 'ADD_CONTENT_TAB',
